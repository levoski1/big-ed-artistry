  ...
  return (
    <div style={{ padding: 40 }}>
      <div style={{ marginBottom: 40, paddingBottom: 28, borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        ...
        <Link href="/custom-artwork" style={{ padding: '12px 24px', fontSize: 12, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', background: 'linear-gradient(135deg, var(--gold-primary), var(--gold-accent))', color: 'var(--text-on-gold)' }}>+ New Order</Link>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 380px' : '1fr', gap: 24, alignItems: 'start' }}>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1fr 1fr 80px', padding: '14px 24px', borderBottom: '1px solid var(--border-color)' }}>
            {['Order', 'Service', 'Price', 'Status', 'Payment', ''].map(h => (
          ...
          {mockOrders.map(order => (
            <div key={order.id} onClick={() => setSelected(selected?.id === order.id ? null : order)} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1fr 1fr 80px', padding: '20px 24px', borderBottom: '1px solid var(--border-color)', alignItems: 'center', cursor: 'pointer', background: selected?.id === order.id ? 'rgba(184,134,11,0.04)' : 'transparent', transition: 'background 0.2s' }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 500 }}>{order.orderNumber}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{formatDate(order.createdAt)}</div>
              </div>
              <div style={{ fontSize: 13, textTransform: 'capitalize', color: 'var(--text-secondary)' }}>{order.service}</div>
              <div style={{ fontSize: 14, color: 'var(--gold-light)', fontWeight: 500 }}>{formatPrice(order.price)}</div>
          ...
          {mockOrders.length === 0 && (
            <div style={{ padding: 80, textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <Link href="/custom-artwork" style={{ display: 'inline-flex', marginTop: 20, padding: '12px 24px', fontSize: 12, letterSpacing: '0.12em', textTransform: 'uppercase', background: 'linear-gradient(135deg, var(--gold-primary), var(--gold-accent))', color: 'var(--text-on-gold)' }}>Place First Order</Link>
            </div>
          )}
        </div>
        ...
        {selected && (
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', position: 'sticky', top: 24 }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 20 }}>{selected.orderNumber}</div>
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ marginBottom: 28 }}>
                ...
                {[
                  { label: 'Order Placed', done: true, date: formatDate(selected.createdAt) },
                  { label: 'Payment Verified', done: selected.paymentStatus === 'paid', date: '' },
                  { label: 'In Progress', done: ['in_progress', 'review', 'completed'].includes(selected.status), date: '' },
                  { label: 'Under Review', done: ['review', 'completed'].includes(selected.status), date: '' },
                  { label: 'Completed & Delivered', done: selected.status === 'completed', date: '' },
                ].map((step, i) => (
                  <div key={step.label} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', marginBottom: 12 }}>
                    ...
                    <div>
                      ...
                      {step.date && <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{step.date}</div>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              {selected.paymentStatus === 'unpaid' && (
                <Link href="/dashboard/payments" style={{ display: 'flex', justifyContent: 'center', marginTop: 24, padding: '12px', fontSize: 12, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', background: 'linear-gradient(135deg, var(--gold-primary), var(--gold-accent))', color: 'var(--text-on-gold)' }}>Upload Payment Proof →</Link>
              )}
            </div>
          </div>
        )}
